	...
	do
	{
		...
		switch(op)
		{
			...
			case 6:
					...
					if(aux==ult)//Aqui cogemos y analizamos si se encuentra en la ultima casilla pero que no es NULL
						{
							
							cout<<"No se puede agregar valor en la posicion seleccionada, redirigiendo a la opcion 1"<<endl;
							p=(nodo *)  malloc(sizeof(struct nodo));
							cout<<"Ingrese un valor: ";
							cin>>p->n;
							if(cab==NULL)
					...
					else//Revisa si se llego al final
						{
						if(aux==NULL)//aqui revisa si se salio del arreglo
							cout<<"La posicion ingresada no se encuentra"<<endl;
						else//aqui pasa si, no se salio del arreglo y si no esta en la ultima casilla 
							{
								p=(nodo *)  malloc(sizeof(struct nodo));
